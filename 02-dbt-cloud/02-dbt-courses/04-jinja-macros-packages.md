...
Create a new macros file "macros/clean_stale_models.sql": 
```sql
{#  
...
Run `dbt run-operation clean_stale_models` to use default params, `dbt run-operation clean-Stale_models --args '{'dry_run':'False'}'` to set params. 
